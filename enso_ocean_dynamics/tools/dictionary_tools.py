# -*- coding:UTF-8 -*-
# ---------------------------------------------------------------------------------------------------------------------#
# Basic tools to handle dictionaries
# ---------------------------------------------------------------------------------------------------------------------#
...
# local functions
from enso_ocean_dynamics.tools.default_tools import set_instance
# ---------------------------------------------------#
...
def read_from_dict(dict_i: dict, *args):
    """
    Read value in the dictionary, if keys are incorrect of input is not a dictionary, return None

    Input:
    ------
    :param dict_i: dict
        Dictionary in which the value must be read
    :param args: str
        Non keyword arguments used as keys in the dictionary
    ...
    -------
    : param: Any
        Value read from input dictionary according to given keywords
    """
    # open dictionary keys
    dict_o = copy__deepcopy(dict_i)
    for k in args:
        if isinstance(dict_o, dict) is False or k not in list(dict_o.keys()):
            dict_o = None
            break
        dict_o = dict_o[k]
    return dict_o
