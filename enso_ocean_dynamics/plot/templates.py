...
# local package
from . panels import plot_hovmoeller, plot_main, plot_map
        ...
        for jj, pan in enumerate(list_panels):
            ...
            if grp in list(dict_i.keys()) and pan in list(dict_i[grp].keys()):
                ...
                if fig_title_bool is True:
                    kwarg["title"] = numbering[counter].upper()
                # panel axes
...
def fig_basic_hov(
    ...
    plt.figure(0, figsize=(nbr_c * x_frac, nbr_l * y_frac))
    gs = GridSpec(nbr_l, nbr_c)
    numbering = panel_numbering(len(list_panel_group) * len(panel_names))
    x_position, y_position = 0, 0
    counter = 0
    for ii, grp in enumerate(list_panel_group):
        list_panels = [k for k in panel_names if k in list(dict_i[grp].keys())]
        for jj, pan in enumerate(list_panels):
            ...
            if grp in list(dict_i.keys()) and pan in list(dict_i[grp].keys()):
                ...
                # kwarg["y_lab"],  _, = figure_axis(kwarg["y_tic"], axis_name="time")
                # plot
                ax = plt.subplot(gs[y_position: y_position + y_size, x_position: x_position + x_size])
                plot_hovmoeller(ax, **kwarg)
            if len(list_panels) > 1 and panel_position == "bottom":
                y_position += y_size + y_delt
            else:
                x_position += x_size + x_delt
    ...
    for ii, grp in enumerate(list_panel_group):
        ...
        for jj, pan in enumerate(list_panels):
            ...
            if grp in list(dict_i.keys()) and pan in list(dict_i[grp].keys()):
                ...
                lon = kwarg["central_longitude"] if "central_longitude" in list(kwarg.keys()) else 0
                crs180 = ccrs.PlateCarree(central_longitude=lon)
                if "x_lim" in list(kwarg.keys()):
                    kwarg["x_lim"] = [k - lon for k in kwarg["x_lim"]]
                if "x_tic" in list(kwarg.keys()):
                    kwarg["x_tic"] = [k - lon for k in kwarg["x_tic"]]
                # plot
                ax = plt.subplot(
